...
export const ControlPanel = () => {
  ...
  const toggleOverlay = async (overlayId: string) => {
    ...
    if (isConnected) {
      if (newEnabledState) {
        ...
        if (createOverlay) {
          try {
            ...
            console.log(
              `Created overlay: ${overlay.name}, windowId:`,
              windowId
            );
            ...
            setOverlays((prev) =>
              prev.map((o) =>
                o.id === overlayId ? { ...o, enabled: true, windowId } : o
              )
            );
          } catch (error) {
            ...
            setOverlays((prev) =>
              prev.map((o) =>
                o.id === overlayId ? { ...o, enabled: true } : o
              )
            );
          }
        } else {
          ...
          setOverlays((prev) =>
            prev.map((o) => (o.id === overlayId ? { ...o, enabled: true } : o))
          );
        }
      } else {
        ...
        if (overlay.windowId && closeOverlay) {
          try {
            ...
            console.log(
              `Closed overlay: ${overlay.name}, windowId: ${overlay.windowId}`
            );
      ...
      setOverlays((prev) =>
        prev.map((o) =>
          o.id === overlayId ? { ...o, enabled: newEnabledState } : o
        )
      );
    }
  };
    ...
    for (const overlay of enabledOverlays) {
      if (updateOverlayProperties) {
        try {
          ...
          console.log(
            `Updated overlay ${overlay.name} with anchor mode ${newAnchorModeState} and position:`,
            newPositions[overlay.id] || overlay.position
          );
        } catch (error) {
  ...
  return (
    <div className="control-panel">
      ...
      <div className="titlebar">
        <img src={dragHandle} alt="Drag" className="titlebar-icon" />
        <h1 className="titlebar-heading">Race Engineer Control Panel</h1>
      </div>
