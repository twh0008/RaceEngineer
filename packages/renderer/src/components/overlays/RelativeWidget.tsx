...
export const RelativeWidget = () => {
    ...
    const initializeDrivers = () => {
      ...
      for (let i = 0; i < 5; i++) {
        if (i === 2) {
          ...
          driversAroundPlayer.push(
            shuffledDrivers[driverIndex] || driverPool[driverIndex]
          );
  ...
  useEffect(() => {
    const updateRaceData = () => {
      if (driversRef.current.length === 0 || gapsRef.current.length === 0)
        return;
      ...
      for (let i = 0; i < newGaps.length; i++) {
        ...
        if (Math.abs(newGaps[i]) < 0.1 && Math.random() < 0.2) {
          ...
          if (isAhead && newGaps[i] > 0) {
            ...
            setTimeout(
              () => setPlayerPosition((prev) => Math.min(20, prev + 1)),
              1000
            );
          }
        }
    ...
    <div className="relative-widget">
      <div className="relative-widget__container">
        <div className="relative-widget__header">
          <h3 className="relative-widget__title">Relative Positions</h3>
        </div>
          ...
          <tbody>
            {relativeData.map((driver, idx) => (
              ...
              >
                ...
                <td
                  className={`relative-widget__cell relative-widget__driver ${
                ...
                <td
                  className={`relative-widget__cell relative-widget__gap ${getGapClassName(driver.gap)}`}
                >
                  {driver.gap}
                </td>
              </tr>
