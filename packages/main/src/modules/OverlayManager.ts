...
class OverlayManager implements AppModule {
  ...
  async enable({ app }: ModuleContext): Promise<void> {
    ...
    app.on('browser-window-created', (_, window) => {
      if (
        !this.#mainWindow &&
        !this.#overlayWindows.has(window.id.toString())
      ) {
  ...
  private setupIpcHandlers(): void {
    ...
    ipcMain.handle(
      'create-overlay',
      async (_event, overlayConfig: OverlayConfig) => {
    ...
    ipcMain.handle(
      ...
      async (_event, overlayConfig: OverlayConfig) => {
        console.log(
          'IPC: update-overlay-properties called with:',
          overlayConfig
        );
        try {
  ...
  private async updateOverlayProperties(
    ...
    console.log(
      `Set overlay window movable: ${overlayConfig.anchorMode || false}`
    );
    ...
    if (this.#renderer instanceof URL) {
      ...
      console.log(
        'Reloading overlay URL with new parameters:',
        overlayUrl.href
      );
    ...
    if (overlayConfig.position) {
      console.log(
        `Setting overlay position for ${overlayConfig.id} to:`,
        overlayConfig.position
      );
      overlayWindow.setPosition(
        overlayConfig.position.x,
        overlayConfig.position.y
      );
    } else {
  ...
  async createOverlay(overlayConfig: OverlayConfig): Promise<string> {
    ...
    if (existingWindow) {
      console.log(
        `Overlay ${overlayConfig.id} already exists, updating properties instead of recreating`
      );
  ...
  private loadOverlayPositions(): Record<string, { x: number; y: number }> {
    try {
      if (fs.existsSync(this.#positionsFilePath)) {
        ...
        const positions = JSON.parse(data) as Record<
          string,
          { x: number; y: number }
        >;
...
export function createOverlayManagerModule(
  ...args: ConstructorParameters<typeof OverlayManager>
) {
